...
model.load_state_dict(torch.load("Model2_stn.pth", map_location=torch.device('cpu'), weights_only=True))
...
def predict(image_path):
    ...
    try:
        ...
        # Store prediction for analytics (commented out visualization to prevent threading issues)
        ...
        return 0  # Default to awake state on error
            ...
            if time.time() - timey > 3:
                ...
                try:
                    ...
                    if cropped_face is not None:  # Face successfully detected
                        ...
                        predicted_state = predict(cropped_face)
                        last_pred = predicted_state
                        
                        # Add visual indicator on frame based on prediction
                        status_text = "AWAKE" if predicted_state == 1 else "DROWSY"
                        status_color = (0, 255, 0) if predicted_state == 1 else (255, 0, 0)
                        
                        # Convert back to BGR for drawing text
                    ...
                    # Convert back to BGR if not already done
                    frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
            else:
...
def stuff():
    ...
    Returns:
        ...
        - Current state description ('Awake', 'Drowsy', 'No Data')
    """
    global summ, timeyy
    ...
    if len(prediction) > 0:
        ...
        if len(prediction) >= 10:
            ...
            awake_count = sum(1 for p in recent_predictions if p == 1)
            engagement_percentage = (awake_count / 10) * 100
            message = f'Engagement: {engagement_percentage:.0f}% - Currently {message}'
